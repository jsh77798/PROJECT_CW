...
{
	if (true == GetGlobalCharacter(OwnerComp)->GetStunCheck())
	{
...
bool UBTTask_AIBase::IsLongStunCheck(UBehaviorTreeComponent& OwnerComp)
{
	if (true == GetGlobalCharacter(OwnerComp)->GetLongStunCheck())
	{
		//LongStun = true;
		SetStateChange(OwnerComp, AIState::STUN);
		GetGlobalCharacter(OwnerComp)->SetStunCheck(false);
